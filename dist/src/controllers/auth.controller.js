...
exports.register = (0, asyncWrapper_1.asyncWrapper)(async (req, res, next) => {
    ...
    if (oldUser)
        next(new AppError_1.AppError(400, "fail", "This Email is Already Registered"));
    // create the new user
...
exports.login = (0, asyncWrapper_1.asyncWrapper)(async (req, res) => {
    ...
    if (!user)
        throw new AppError_1.AppError(400, "fail", "Invalid Email or Password");
...
exports.logout = (0, asyncWrapper_1.asyncWrapper)(async (req, res, next) => {
    ...
    if (!payload)
        next(new AppError_1.AppError(400, "fail", "request payload is undefined"));
    const user = await users_model_1.userModel.findOne({ _id: payload.id });
    if (!user)
        return next(new AppError_1.AppError(404, "fail", "User Not Found"));
    res.clearCookie("REFRESH_TOKEN", { secure: true, sameSite: "none" });
    if (!user?.REFRESH_TOKEN)
        next(new AppError_1.AppError(400, "fail", "User Already Logged Out"));
